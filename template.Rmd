...
-=-------
```{r env1}
questionSummary <- gradesheet %>% group_by(Q) %>% summarise(NumCorrect=sum(correct=="Yes",na.rm=T),NumPartial=sum(correct=="Partial",na.rm=T) , NumResponses=n(),ClassMean=mean(scoreFracNum,na.rm=T)) %>% arrange(ClassMean)
